...
const Avatar: React.FC<AvatarProps> = ({ user, size = 'md', onAvatarUpload, loading = false, className = '', showUploadButton = false, isAssistant = false, isLogo = false }) => {
  ...
  return (
    <div className={`relative inline-block ${className}`}>
      ...
      {showUploadButton && onAvatarUpload && (
        ...
        >
          {loading ? <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-white"></div> : <FaCamera className="h-3 w-3" />}
