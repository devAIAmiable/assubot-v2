...
const FAQPage = () => {
  ...
  const questions: FAQQuestion[] = [
    ...
    },
    {
      ...
      answer:
        "Oui ! AI'A analyse vos contrats et vous fournit des arguments de négociation basés sur les conditions de marché et les meilleures pratiques. Il vous aide à identifier les clauses défavorables et vous suggère des points de négociation pour obtenir de meilleures conditions.",
  ...
  const getCategoryStats = (categoryId: string) => {
    return questions.filter((q) => (categoryId === 'all' ? true : q.category === categoryId)).length;
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {/* Search Bar */}
          <motion.div className="max-w-2xl mx-auto mt-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <motion.div className="flex flex-wrap gap-2 justify-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}>
          {categories.map((category) => (
            <button
              key={category.id}
              onClick={() => selectCategory(category.id)}
              className={`flex items-center space-x-2 px-4 py-2 rounded-lg border transition-all duration-200 ${
                activeTab === category.id ? 'bg-[#1e51ab] text-white border-[#1e51ab]' : 'bg-white text-gray-700 border-gray-300 hover:border-[#1e51ab] hover:text-[#1e51ab]'
              }`}
            >
              {category.icon}
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <motion.div className="space-y-3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.6 }}>
          <AnimatePresence mode="wait">
            {filteredQuestions.length === 0 ? (
              <motion.div key="no-results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="text-center py-12">
                <div className="w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                  <FaSearch className="h-6 w-6 text-gray-400" />
                </div>
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Aucune question trouvée</h3>
                <p className="text-gray-600">Essayez de modifier vos critères de recherche</p>
              </motion.div>
            ) : (
              <motion.div key="questions" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-3">
                  ...
                  >
                    ...
                    >
                      <div className="flex-1">
                        <h3 className="text-base font-medium text-gray-900 mb-1">{question.question}</h3>
                      ...
                      <FaChevronDown className={`h-4 w-4 text-gray-400 transition-transform duration-300 ${openQuestions.includes(question.id) ? 'rotate-180' : ''}`} />
                    </button>
                    <AnimatePresence>
      ...
      {/* Contact Section */}
      <motion.div className="bg-white border-t border-gray-200 py-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }}>
