...
const ContractsModule = () => {
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}>
        {filteredContracts.length === 0 ? (
          <div className="bg-white border border-gray-100 rounded-2xl p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">Commencez par ajouter vos premiers contrats d'assurance.</p>
            <button onClick={() => setIsCreateContractModalOpen(true)} className="bg-[#1e51ab] text-white px-6 py-3 rounded-xl font-semibold hover:bg-[#163d82] transition-colors">
              Ajouter un contrat
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredContracts.map((contract, index) => {
              ...
              const isContractExpired = contract.endDate ? new Date(contract.endDate) < new Date() : false;
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                    <div className="flex items-center space-x-2">
                      <button
                        ...
                        disabled={isPending}
                        className={`transition-colors p-1 ${isPending ? 'text-gray-300 cursor-not-allowed' : 'text-gray-400 hover:text-[#1e51ab]'}`}
                      ...
                      <button
                        ...
                        disabled={isPending}
                        className={`transition-colors p-1 ${isPending ? 'text-gray-300 cursor-not-allowed' : 'text-gray-400 hover:text-red-600'}`}
      ...
      {/* Edit Contract Modal */}
      {editingContract && <EditContractModal contract={editingContract} isOpen={!!editingContract} onClose={() => setEditingContract(null)} onSuccess={handleEditSuccess} />}
