	...
	useEffect(() => {
		const processPurchase = async () => {
			...
			try {
				...
				const newCreditBalance = (currentUser?.creditBalance || 0) + purchasedPack.creditAmount;
		...
		processPurchase();
	}, [sessionId, creditPackId, creditPacks, currentUser?.creditBalance, dispatch]);
				...
				>
					...
					{creditsAdded > 0 && (
						...
						>
							...
							<p className="text-gray-600">
								Vos crédits sont maintenant disponibles pour utiliser les fonctionnalités premium.
							</p>
						</motion.div>
					)}
					...
					>
						...
						<div className="mt-8 text-sm text-gray-500">
							...
							{!creditsAdded && (
								<p className="mt-1">Vos crédits sont disponibles dans votre compte.</p>
							)}
						</div>
					</motion.div>
				</motion.div>
