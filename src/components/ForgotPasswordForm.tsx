...
const ForgotPasswordForm: React.FC = () => {
  ...
  const onSubmit = async (data: ForgotPasswordFormData) => {
    ...
    try {
      ...
      if (response.success) {
        trackForgotPasswordRequest({ status: 'success' });
        setSubmitSuccess('Un email de réinitialisation a été envoyé à votre adresse email. Veuillez vérifier votre boîte de réception.');
        reset();
      } else {
        trackForgotPasswordRequest({ status: 'error', errorMessage: response.error });
        setSubmitError(response.error || "Erreur lors de l'envoi de l'email");
      }
    } catch (error) {
      console.error('Forgot password error:', error);
      trackForgotPasswordRequest({ status: 'error', errorMessage: 'Erreur de connexion au serveur' });
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">Entrez votre adresse email pour recevoir un lien de réinitialisation</p>
        </div>
