...
const ContratsModule = () => {
	...
	const { 
		...
		loading,
		error
	} = useAppSelector((state) => state.contracts as any);
	...
	const getTypeIcon = (type: string) => {
		switch (type) {
			case 'auto': return FaFileContract;
			case 'habitation': return FaFileContract;
			case 'sante': return FaFileAlt;
	...
	const contractStats = {
		...
		totalPremium: contracts.filter(c => c.status === 'active').reduce((sum, c) => sum + c.premium, 0),
	};
				...
				<div className="bg-white border border-gray-100 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-3xl font-bold text-gray-900">{contractStats.totalPremium.toLocaleString()}€</p>
						</div>
