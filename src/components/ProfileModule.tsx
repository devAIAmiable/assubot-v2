import { FaCalendarAlt, FaCheck, FaEdit, FaEnvelope, FaExclamationTriangle, FaGoogle, FaLock, FaMapMarkerAlt, FaSave, FaUserShield, FaTimes, FaUser } from 'react-icons/fa';
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        ...
        <p className="text-gray-600 text-lg">Gérez vos informations personnelles et paramètres de compte</p>
      </motion.div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-3 flex-1">
              ...
              <p className="text-sm text-yellow-800 mt-1">
                Veuillez compléter :{' '}
                {[!currentUser?.birthDate && 'date de naissance', !currentUser?.gender && 'genre', !currentUser?.professionalCategory && 'catégorie professionnelle']
                  .filter(Boolean)
                  .join(', ')}
              </p>
            </div>
      ...
      >
        <div className="flex flex-col md:flex-row items-center md:items-start space-y-6 md:space-y-0 md:space-x-8">
          {/* Avatar */}
          <Avatar user={currentUser} size="xl" onAvatarUpload={handleAvatarUpload} loading={avatarLoading} showUploadButton={true} />

          {/* User Info */}
          <div className="flex-1 text-center md:text-left">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">{`${currentUser.firstName} ${currentUser.lastName}`}</h2>
            <p className="text-gray-600 mb-1">{currentUser.email}</p>
            {currentUser.professionalCategory && <p className="text-gray-500 mb-4">{getProfessionalCategoryLabel(currentUser.professionalCategory)}</p>}
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          {!isEditingPersonal && (
            <Button variant="ghost" size="sm" onClick={() => setIsEditingPersonal(true)} className="flex items-center">
        ...
        {isEditingPersonal ? (
          <form onSubmit={personalForm.handleSubmit(handlePersonalSubmit)} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              />
              <Input label="Nom" type="text" {...personalForm.register('lastName')} required disabled={profileLoading} error={personalForm.formState.errors.lastName?.message} />
              <Input
            ...
            <div className="flex space-x-3">
              <Button type="submit" className="flex items-center" disabled={profileLoading || !isPersonalFormValid}>
                {profileLoading ? (
                  <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="font-medium text-gray-900">{currentUser.firstName || 'Non renseigné'}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium text-gray-900">{currentUser.birthDate ? formatDateForDisplay(currentUser.birthDate) : 'Non renseigné'}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600 mb-1">Genre</p>
              <p className="font-medium text-gray-900">
                {currentUser.gender === 'male' ? 'Homme' : currentUser.gender === 'female' ? 'Femme' : currentUser.gender === 'other' ? 'Autre' : 'Non renseigné'}
              </p>
            </div>
            <div className="md:col-span-2">
              <p className="text-sm text-gray-600 mb-1">Catégorie professionnelle</p>
              <p className="font-medium text-gray-900">{getProfessionalCategoryLabel(currentUser.professionalCategory)}</p>
            </div>
          </div>
        )}
      </motion.div>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          {!isEditingAddress && (
            <Button variant="ghost" size="sm" onClick={() => setIsEditingAddress(true)} className="flex items-center">
              <FaEdit className="h-4 w-4 mr-1" />
              Modifier
            </Button>
          )}
        ...
        {isEditingAddress ? (
          <form onSubmit={addressForm.handleSubmit(handleAddressSubmit)} className="space-y-4">
            <Input label="Adresse" type="text" {...addressForm.register('address')} placeholder="123 Rue de la République" error={addressForm.formState.errors.address?.message} />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input label="Ville" type="text" {...addressForm.register('city')} placeholder="Paris" error={addressForm.formState.errors.city?.message} />
              <Input label="Code postal" type="text" {...addressForm.register('zip')} placeholder="75001" error={addressForm.formState.errors.zip?.message} />
            </div>
            <div className="flex space-x-3">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="font-medium text-gray-900">
                ...
                {currentUser.city && `${currentUser.zip ? currentUser.zip + ' ' : ''}${currentUser.city}`}
              </p>
            </div>
          </div>
        )}
      ...
      >
        ...
        <div className="space-y-6">
          ...
          <div className="border-b border-gray-100 pb-6">
            ...
            <p className="text-sm text-gray-500 mt-1">L'adresse email ne peut pas être modifiée pour des raisons de sécurité.</p>
          </div>
          ...
          <div>
            ...
            {isChangingPassword ? (
              <form onSubmit={passwordForm.handleSubmit(handlePasswordSubmit)} className="space-y-4">
                ...
                />
                <div className="flex space-x-3">
                  <Button type="submit" size="sm" disabled={passwordLoading || !passwordForm.formState.isValid}>
                    {passwordLoading ? (
                      <>
                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                        Changement...
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p className="font-medium text-gray-900">{currentUser.createdAt ? new Date(currentUser.createdAt).toLocaleDateString('fr-FR') : 'Non disponible'}</p>
          </div>
          <div>
            <p className="text-gray-600 mb-1">Dernière modification</p>
            <p className="font-medium text-gray-900">{currentUser.updatedAt ? new Date(currentUser.updatedAt).toLocaleDateString('fr-FR') : 'Non disponible'}</p>
          </div>
        </div>
      </motion.div>
      ...
      {currentUser?.email?.endsWith('@a-lamiable.com') && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="mt-8 pt-6 border-t border-gray-100">
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200">
            <div className="flex items-center justify-between">
              ...
              <Button variant="primary" size="sm" onClick={() => navigate('/app/admin')} className="bg-blue-600 hover:bg-blue-700">
      ...
      {/* Legal Footer */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.6 }} className="mt-8 pt-6 border-t border-gray-100">
        <div className="flex flex-col sm:flex-row items-center justify-between space-y-2 sm:space-y-0">
          <p className="text-sm text-gray-500">© {new Date().getFullYear()} AssuBot. Tous droits réservés.</p>
          <div className="flex items-center space-x-6">
            <button onClick={() => navigate('/general-terms')} className="text-sm text-gray-600 hover:text-[#1e51ab] transition-colors">
              Conditions d'utilisation
            </button>
            <button onClick={() => navigate('/privacy-policy')} className="text-sm text-gray-600 hover:text-[#1e51ab] transition-colors">
