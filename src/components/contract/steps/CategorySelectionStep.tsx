...
const CategorySelectionStep: React.FC<CategorySelectionStepProps> = ({ onDataUpdate, initialData }) => {
  // Categories configuration from centralized config
  const categories = Object.entries(CATEGORY_CONFIG).map(([id, config]) => ({
    id: id as ContractCategory,
    name: config.label,
    icon: config.icon,
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
      <div className="text-center mb-8">
        <h3 className="text-2xl font-semibold text-gray-900 mb-2">Choisissez le type d'assurance</h3>
            ...
            return (
              <motion.button
                ...
                className={`p-6 rounded-xl border-2 transition-all duration-200 ${
                  isSelected ? 'border-[#1e51ab] bg-blue-50 text-[#1e51ab] shadow-lg' : 'border-gray-200 hover:border-gray-300 text-gray-600 hover:shadow-md'
                }`}
              >
