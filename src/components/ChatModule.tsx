...
const ChatModule: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Optimistic updates and credit handling are now managed in the API layer
  ...
  const handleQuickAction = async (action: QuickAction) => {
    ...
    try {
      // Clear quick actions immediately for better UX
      clearChatQuickActions(currentChat.id);

      // Send the action as user message
      // Quick actions will be updated by the API layer
  ...
  return (
    <div className="flex bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden h-full" style={{ height: 'calc(100vh - 140px)' }}>
      ...
      <div className={`w-full md:w-1/3 bg-white border-r border-gray-200 flex flex-col ${showChatList ? 'flex' : 'hidden md:flex'}`}>
        {/* Header */}
        <div className="bg-[#1e51ab] p-4 border-b border-gray-200">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-white">Conversations</h2>
            <button onClick={() => setShowNewChatModal(true)} className="p-2 text-white hover:text-gray-200 hover:bg-[#1a4599] rounded-full transition-colors">
              <FaPlus />
            </button>
          </div>
              ...
              {paginatedChats.map((chat) => (
                <div
                  ...
                  className={`group relative mx-3 my-1 rounded-xl cursor-pointer transition-all duration-200 ${
                    currentChat?.id === chat.id ? 'bg-blue-50 border border-blue-200 shadow-sm' : 'hover:bg-gray-50 hover:shadow-sm'
                  }`}
                    ...
                    <div className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-1">
                          <button onClick={() => handleUpdateChatTitle(chat.id)} className="p-2 text-green-600 hover:text-green-700 bg-gray-100 rounded-lg transition-colors">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between">
                            <div className="flex-1 min-w-0">
                              <h3 className={`font-semibold text-base truncate mb-1 ${currentChat?.id === chat.id ? 'text-blue-900' : 'text-gray-900'}`}>{chat.title}</h3>
                              <div className={`text-sm truncate ${currentChat?.id === chat.id ? 'text-blue-600' : 'text-gray-500'}`}>
                            ...
                            <div className="flex flex-col items-end gap-2">
                              <span className={`text-xs ${currentChat?.id === chat.id ? 'text-blue-500' : 'text-gray-400'}`}>
                              ...
                              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                  ...
                                  className={`p-2 rounded-full transition-colors ${
                                    currentChat?.id === chat.id ? 'text-blue-600 hover:text-blue-800 hover:bg-blue-100' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100'
                                  }`}
                                >
                                ...
                                <button
                                  ...
                                  className={`p-2 rounded-full transition-colors ${
                                    currentChat?.id === chat.id ? 'text-blue-600 hover:text-red-600 hover:bg-red-50' : 'text-gray-400 hover:text-red-600 hover:bg-red-50'
                                  }`}
                                >
        ...
        {currentChat ? (
          <>
            ...
            <div className="bg-[#1e51ab] p-4 border-b border-gray-200 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <button onClick={handleBackToChatList} className="md:hidden p-2 text-blue-200 hover:text-white hover:bg-blue-600 rounded-full transition-colors">
                  <FaArrowLeft />
                </button>
                          ...
                          <div className="text-sm prose prose-sm max-w-none">
                            <ReactMarkdown
                              ...
                              components={{
                                ...
                                th: ({ children, align }) => (
                                  <th className={`px-4 py-3 text-left font-semibold  ${align === 'right' ? 'text-right' : align === 'center' ? 'text-center' : 'text-left'}`}>
                                    {children}
                                  </th>
                                ),
                                td: ({ children, align }) => (
          ...
          <div className="flex-1 flex items-center justify-center bg-gray-50">
            <div className="text-center max-w-md px-4">
              ...
              <button onClick={() => setShowNewChatModal(true)} className="bg-[#1e51ab] text-white px-6 py-3 rounded-lg hover:bg-[#1a4599] transition-colors font-medium">
                Nouvelle conversation
              </button>
            </div>
          </div>
          ...
          >
            ...
            >
              ...
              <div className="flex justify-end gap-3">
                <button onClick={cancelDeleteChat} className="px-4 py-2 text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded-lg transition-colors font-medium">
                  Annuler
                </button>
                <button
        ...
        {error && (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <span className="font-medium text-sm">{typeof error === 'string' ? error : 'Une erreur est survenue'}</span>
              <button onClick={clearChatError} className="p-1 text-red-200 hover:text-white hover:bg-red-600 rounded transition-colors">
                <FaTimes />
              </button>
            </div>
          </motion.div>
