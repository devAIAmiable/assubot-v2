...
export const useComparisons = () => {
	...
	const saveComparison = useCallback((comparisonData: Omit<PastComparison, 'id' | 'date' | 'expiresAt'>) => {
	...
	const getComparisonsByType = useCallback((type: string) => {
		return validComparisons.filter(comparison => comparison.insuranceType === type);
	}, [validComparisons]);

	const getRecentComparisons = useCallback((limit = 5) => {
		return validComparisons.slice(0, limit);
	}, [validComparisons]);

	// Pagination utilities
	const getPaginatedComparisons = useCallback((page: number, itemsPerPage: number) => {
