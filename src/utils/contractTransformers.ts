...
export function transformBackendTermination(backendTermination: BackendContractCancellation): ContractCancellation {
  return {
    ...
    contractId: '', // Will be set by parent contract
    question: backendTermination.question,
    response: backendTermination.response,
    createdAt: new Date(), // Backend doesn't provide this, use current date
  };
}
...
export function transformBackendContracts(backendContracts: BackendContract[]): ContractWithRelations[] {
  return backendContracts.map((contract) => transformBackendContract(contract as GetContractByIdResponse));
}

/**
 * Transform a backend contract list item to frontend format
 */
export function transformBackendContractListItem(backendContract: BackendContractListItem): ContractListItem {
  return {
    id: backendContract.id,
    name: backendContract.name,
    insurerName: backendContract.insurerName,
 ...
 */
export function transformBackendContractListItems(backendContracts: BackendContractListItem[]): ContractListItem[] {
