...
export const authService = {
  ...
  signup: async (userData: {
    ...
    try {
      const response = await coreApi.post<{ user: unknown; message: string }>('/auth/register', userData);
...
export const contractsService = {
  ...
  update: (id: string, contractData: unknown) => coreApi.put<unknown>(`/contracts/${id}`, contractData),
  delete: (id: string) => coreApi.delete(`/contracts/${id}`),
  uploadDocument: (contractId: string, documentData: FormData) => coreApi.post<unknown>(`/contracts/${contractId}/documents`, documentData),
...
export const notificationsService = {
  ...
  updatePreferences: (preferences: unknown) => coreApi.put<unknown>('/notifications/preferences', preferences),
};
...
export const userService = {
  ...
  changePassword: async (passwordData: { currentPassword: string; newPassword: string }): Promise<ServiceResponse<{ message: string }>> => {
    try {
      const response = await coreApi.patch<{ message: string }>('/users/change-password', passwordData);
