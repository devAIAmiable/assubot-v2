  ...
  async uploadContract(
    ...
    try {
      ...
      const uploadPromises = uploadUrlsResponse.uploadUrls.map(async (urlData, index) => {
        ...
        try {
          // Upload to Azure
          await this.uploadToAzureWithProgress(urlData.uploadUrl, files[index].file);
          ...
          uploadResults[resultIndex].error = error instanceof Error ? error.message : 'Upload failed';
          throw error;
        }
      });
      ...
      const contractResult = await store.dispatch(contractsApi.endpoints.initContract.initiate(contractInitData)).unwrap();
