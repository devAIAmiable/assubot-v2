...
export const step3Schema = z.object({
	documents: z
		...
		.refine(
			(docs) => docs.some((doc) => doc.type === 'CP'),
			'Les conditions particulières (CP) sont obligatoires'
		)
		.refine(
			(docs) => docs.some((doc) => doc.type === 'CG'),
			'Les conditions générales (CG) sont obligatoires'
		)
		.refine(
			(docs) => docs.every((doc) => doc.fileSize <= 10 * 1024 * 1024),
			'Chaque fichier ne peut pas dépasser 10MB'
		)
		.refine(
			(docs) =>
				docs.every((doc) =>
